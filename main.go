...
var (
	...
	h          = flag.Bool("h", false, "")
	out        = flag.String("out", ".", "Output put directory")
...
func main() {
	...
	if *h || *help {
		fmt.Printf("sscg - %v\n", VERSION)
		fmt.Println("")
		fmt.Println("Generate self signed certificate.")
		fmt.Println("")
		flag.PrintDefaults()
		fmt.Println("")
		return
	...
	if _, err := os.Stat(outDirectory); os.IsNotExist(err) {
		os.Mkdir(outDirectory, 0700)
	}

	certPath := path.Join(outDirectory, "cert.pem")
